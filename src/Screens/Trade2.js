...
class TradeScreen extends React.Component {
          ...
          <PTRView onRefresh={()=>this.refresh()} >
			  <ScrollView showsVerticalScrollIndicator={false}>
				...
				<View style={[{height: 44, marginLeft: -15, marginRight: -15, alignItems: 'center', flexDirection: 'row', backgroundColor: darkmode?'black':'white'}, styles.topTabBar]}>
					...
					<TouchableHighlight onPress={() => this.setTab(index)} style={[styles.topTab, currentTab === index && styles.activeTab]} key={index}>
						<Image source={item.Image} style={{width:25, height:25, alignItems:'center',alignSelf:'center'}}></Image>							
					</TouchableHighlight>)}
				</View>
				<View >
					<Text style={{color:txtColor,fontSize:15,alignItems:'center',alignSelf:'center',marginTop:20}}>Current Balance</Text>
					<View style={{flexDirection:'row'}}>
						<SideTrade label={'Receive'} icon={'trending-down-outline'} position={'left'}
							onPress={this.toggleModal}
						/>
						<View style={{flex:1}}>
							<Text style={{fontSize:23,marginTop:20,color:txtColor,alignSelf:'center'}}>{currency_data[currentTab][1].toFixed(currency_data[currentTab][5])} {Headers[currentTab]['text']}</Text>
							<Text style={{fontSize:20,marginTop:5,color:txtColor,alignSelf:'center'}}>${currency_data[currentTab][3].toFixed(2)}</Text>
						</View>
						<SideTrade label={'Send'} icon={'trending-up-outline'} position={'right'}
							onPress={this.toggleSendModal}/>
				...
				<History label={'Activity'} data={this.state.history} darkmode={darkmode}
                        isLoad={!this.state.history_finish}/>
				
			<Modal isVisible={this.state.show_receive_modal} style={{margin:0}}
  ...
  handleBackButton(){
	  ...
	  if(this.state.qr_code_modal){
		  this.setState({qr_code_modal:false})
		  return true;
	  }
